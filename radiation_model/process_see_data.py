...
# Make sure to download the SOHO/ACE/GOES/STEREO data
### 🔹 EPAM Data Processing ###
def read_epam(
    input_file=FLUX_DIR / "exported_data.txt", output_file=FLUX_DIR / "epam.csv"
):
    ...
    # Apply Savitzky-Golay filter to all columns
    epam_cols = [
        col
        for col in df.columns
        if (
            col.startswith(("P", "DE", "W", "E", "FP", "Z"))
            and not col.startswith("unc_")
        )
    ]
    ...
    df = process_timeseries(
        df,
        instrument="ACE EPAM",
        cols_to_use=epam_cols,
        invalid_markers=-999.9,
        sampling_time=5,
        apply_savgol=False,
    )
...
### 🔹 STEREO/HET Data Fetching & Processing ###
def get_stereo_het_data(url, output_file):
    ...
    # Define columns for electrons and protons
    ...
    df = process_timeseries(
        df,
        instrument="STEREO",
        cols_to_use=flux_columns,
        invalid_markers=-999,
        sampling_time=5,
    )

    # Create datetime column from index
    df["datetime"] = df.index
...
def read_soho_erne(date):
    ...
    try:
        with pycdf.CDF(str(filename)) as cdf:
            ...
            proton_energies, proton_delta = (
                cdf["P_energy"][...],
                cdf["P_energy_delta"][...],
            )

            for i, (energy, delta) in enumerate(zip(proton_energies, proton_delta)):
                df[f"H_{energy - delta/2:.1f}-{energy + delta/2:.1f}MeV"] = cdf["PH"][
                    ...
                ][:, i]
...
# Filter peaks and valleys in the data
def process_timeseries(
    df,
    instrument="EPAM",
    time_col="datetime",
    invalid_markers=None,
    cols_to_use=None,
    ...
    # Clean invalid data
    df_clean = (
        df_subset.replace(invalid_markers, np.nan) if invalid_markers else df_subset
    )
    ...
    if apply_savgol:
        ...
        # Resample to 1-minute intervals before smoothing
        df_1min = df_clean.resample("1T").mean()

        # Auto-adjust window size (ensure it's odd)
        if window_size is None:
            window_size = max(5, 11)  # Default to 11 for a 1-minute dataset
        if window_size % 2 == 0:
        ...
        # Convert back to DataFrame with the same index
        df_savgol = pd.DataFrame(
            df_savgol, index=df_1min.index, columns=df_1min.columns
        )

        # Resample to final 5-minute intervals (or any chosen frequency)
        df_resampled = df_savgol.resample(f"{sampling_time}T").mean()

    else:
        # No Savitzky-Golay filtering/resampling; adjust datetime index if needed.
